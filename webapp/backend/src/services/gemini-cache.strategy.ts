...
export const GEMINI_CACHE_TTL = {
  ...
  // Chat responses - cache for 15 minutes (shorter as conversations are dynamic)
  chat: 900,

  // Shot suggestions - cache for 1 hour (relatively stable)
...
export function generateGeminiCacheKey(
  ...
  // For shot suggestion queries
  if (prefix === 'shot-suggestion' && params.sceneDescription && params.shotType) {
    const sceneHash = crypto
      .createHash('sha256')
      .update(params.sceneDescription)
      .digest('hex')
      .substring(0, 12);
    const shotTypeHash = crypto
      .createHash('sha256')
      .update(params.shotType)
      .digest('hex')
      .substring(0, 8);

    return `${keyPrefix}:${shotTypeHash}:${sceneHash}`;
  }
