...
export class ProjectsController {
  ...
  async analyzeScript(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      // Import at the top would be better, but inline import works for now
      const { geminiService } = await import('@/services/gemini.service');

      try {
        // Perform character analysis
        const characterAnalysis = await geminiService.analyzeText(
          project.scriptContent,
          'characters'
        );

        // Perform structure analysis
        const structureAnalysis = await geminiService.analyzeText(
          project.scriptContent,
          'structure'
        );

        // Perform theme analysis
        const themeAnalysis = await geminiService.analyzeText(
          project.scriptContent,
          'themes'
        );
        ...
        logger.info('Script analysis completed', { projectId: id });
      } catch (analysisError) {
        logger.error('AI analysis failed:', analysisError);
        res.status(500).json({
          success: false,
          error: 'فشل تحليل السيناريو باستخدام الذكاء الاصطناعي',
        });
      }
    } catch (error) {
